...
- Drop analysis, as ported (Strahler order recomputed per threshold): CA objective 120 cells (0.0951 km2), band 120 to 600, the same as the Toolkit's corrected objective on the same DEM; on AR the port has isolated passes at 60 to 70 and 160 to 170 cells and its first sustained band starts at 320 cells with 8 streams left, while the Toolkit's `drop_analysis.py` on the same DEM (`elev_clipped.tif`, sha256 a0e570a8..) selects 150 cells from a band of exactly three thresholds (150 to 170); the two sweeps agree at every threshold except 150 cells, where |t| is 1.97 in the Toolkit and 2.05 in the port, so on a basin this small the selection hinges on a single threshold (corrected 2026-09-24: this sentence first said the Toolkit had no usable AR objective either, before the corrected Toolkit sweep had been run on the AR 28 m grid). One arithmetic note: the sweep thresholds the upstream cell count with `>=`, while `extractNetwork` thresholds the upstream area in m2, so 47571.5 m2 excludes the three CA cells whose count is exactly 60 (269 stream cells in the sweep row, 266 in the network).
