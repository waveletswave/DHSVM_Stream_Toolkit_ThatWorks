...
- `docs/audit/`: the audit trail. The standalone rebuild inventory and the per-stage audits (slope conditioning, soil-depth sensitivity, the Tier B unit fix, the Tier E network orientation fix, the drop-analysis Strahler cache, the cross-engine comparison with WW-DHSVM).
...
The stream network is no longer validated against the QGIS reference: in September 2026 the network stage shared by both implementations was found to write every segment upstream, with a headwater as the outlet, and was rebuilt from the stream raster and its flow-direction raster (Tier E). The corrected network is verified by invariants (every segment drains downhill to a basin mouth, dense stream order, one map record per stream cell) and by DHSVM reruns of the manuscript configurations, in which the network moves three-year streamflow by about 0.005 percent at most and the evaluation metrics by 0.001 to 0.002 over the full period (at most 0.003 in any single year). The full record is in `standalone_CA/docs/validation_log.md` and `docs/audit/tier_e_network_orientation_2026_09_22.md`. The corrected network was then compared with the one WW-DHSVM (an independent engine: pyflwdir D8, its own segment builder) draws on the same grid, DEM and support area: the two agree on the trunk and differ in the headwaters, and DHSVM run on the manuscript inputs with either network gives three-year streamflow within 0.08 percent and metrics within 0.007 NSE (`docs/audit/cross_engine_comparison_2026_09_23.md`, `scripts/diagnostics/cross_engine/`).
