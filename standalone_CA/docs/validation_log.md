...
Superseded 2026-09-22: the network this section validated is reversed (every segment points upstream and the written outlet is a headwater), on the reference as much as on the standalone. See the Tier E section at the end and docs/audit/tier_e_network_orientation_2026_09_22.md. The text below is kept as the record of what was checked and what was missed.
...
Superseded 2026-09-22: both networks compared here were reversed; the d=0 tie-break question is moot under the raster-native stage (Tier E section below).
...
## Stream network orientation (Tier E): raster-native network stage

The network stage was found to write every segment the wrong way, on the QGIS reference and on the standalone alike. r.watershed on a basin-clipped DEM writes negative accumulation on every cell; vector_attrs.py took the endpoint with the larger raw value as downstream, i.e. the smaller magnitude, and stream_network.py then merged the resulting sinks, which made a headwater at the initiation threshold the written outlet (segment 41, down 0, |acc| 60.5 at its "downstream" end). None of the checks above could see it: the isomorphism check compared the two pipelines with each other, the end-to-end DHSVM validation compared them with each other, and the manuscript's simulated Q is the channel lateral inflow, which does not depend on routing. The full record, with numbers, is docs/audit/tier_e_network_orientation_2026_09_22.md.

The stage is now raster-native: run_hydrology_grass.sh exports the r.stream.extract direction raster (stream_dir.tif); segments_from_raster.py walks it (segments start at heads and below confluences, end where the successor leaves the raster; Kahn order gives a dense propagated rank; slope, meanmsq, length and aspect keep the earlier conventions); channelclass_standalone.py is unchanged; network_files.py checks the topology invariants and writes stream.network.dat with SAVE on every outlet row, and stream.map.dat. vector_attrs.py, stream_network.py and iso_check_network.py are retired to scripts/legacy/standalone_CA_pre_tierE/. Six synthetic tests (standalone_CA/tests) cover the stage without GRASS; check_network_orientation.py is the read-only diagnostic.

Verdict on CA 28 m (DCC, from-scratch run at 71b8a4c against the 1aaee3d baseline): grid binaries and grid states byte-identical; stream.class.dat, stream.map.dat, stream.network.dat and Channel.State changed as designed. 22 segments (was 41, 19 of them one cell long), 231 map records on 231 cells (was 275 on 235, with 29 cells under two segments and 4 off the raster), two mouth outlets (was one headwater), classes 13 and 14 (was 13 throughout, because the meanmsq sampler kept only positive accumulation and always fell back to the cell area), ranks dense 1 to 8, total length 7600.8 m (was 7526.0; exact D8 steps and a full cell at each outlet). Diagnostic: 0 hard invariants failed.

DHSVM (Mac, DHSVM3.2 of 2025-01-13, the manuscript's CA_S4h, CA_LAI70 and CA_LAI20 configurations): the control runs on the April input set reproduce the April outputs byte for byte; with only the network swapped, three-year ET moves by 0.03 to 0.09 mm, ChannelInt by 0.03 to 0.10 mm, the daily lateral inflow by at most 0.06 mm/d (RMSE 0.005 mm/d, r 0.999998), and the manuscript's stitched metrics from 0.681 / 0.841 / 2.310 / -0.3 (NSE, r, RMSE, PBIAS) to 0.682 / 0.841 / 2.309 / -0.3; the per-year optimal leaf-area multipliers do not move. DHSVM takes the two SAVE outlet rows, and their sum equals the Stream.Flow total outflow to the printed precision (R12); the routed outflow, zero in the April runs because the pre-Tier-A network never routed a segment, is now usable (ratio to ChannelInt 1.0000). The hourly routed hydrograph changes by up to 8% of the peak, which is the correct direction and the two outlets at work.

Two other things surfaced on the way and are recorded in the audit: the April manuscript inputs differ from the June 8 outputs in soildepth.bin (slope unit fix plus slope conditioning) and stream.network.dat (Tier A and B), and the June inputs move the same stitched metrics to 0.683 / 0.841 / 2.307 / +0.4 with the old network and 0.683 / 0.842 / 2.305 / +0.4 with Tier E; and this DHSVM build crashes when the Output Directory value exceeds 78 characters (100-byte file name buffers in InitDump.c and RouteSubSurface.c).
...
## Cross-engine comparison with WW-DHSVM (2026-09-23)

The Tier E network was compared with the network WW-DHSVM (Zhi Li, UConn; pyflwdir priority-flood fill and D8, its own segment builder and basin-scaled class table) draws on the same grid, DEM and support area (A_c = 60 cells, no burning), with `scripts/diagnostics/cross_engine/compare_engines.py` against the fork branch `feat-channel-initiation`. CA: 231 cells / 22 segments (Toolkit) against 266 / 30 (WW-DHSVM), 153 common cells, 84 of the 97 Toolkit cells above 300 cells of MFD accumulation shared, the disagreement in the headwaters where D8 concentrates what MFD spreads; the Toolkit's two mouth outlets against WW-DHSVM's one at (68, 68). AR: 170 / 19 against 203 / 29, 151 common, the same outlet cell. Both WW-DHSVM networks pass the Toolkit diagnostic (0 hard failures) and WW-DHSVM's own checks; the ported drop analysis gives the same CA objective (120 cells) and no usable AR objective (the Toolkit's own sweep selects 150 cells on AR; the two sweeps differ at a single threshold, 150 cells, where |t| straddles 2). DHSVM with the WW-DHSVM network on the April manuscript inputs (`wA_S4h`, `wA_LAI70`, `wA_LAI20`, AR `wA_S4h`): three-year Q moves by 0.015 to 0.077% against Tier E (daily RMSE 0.03 to 0.05 mm/d, r 0.9998 to 0.9999), the CA stitched metrics from 0.682 / 0.841 / 2.309 / -0.3 to 0.681 / 0.841 / 2.311 / -0.3 and the AR overall metrics from 0.601 / 0.777 / 1.962 / -3.5 to 0.596 / 0.774 / 1.975 / -3.5; R12 holds to floating-point precision on the single WW-DHSVM outlet. A change of network engine does not reach the manuscript's numbers. Record: docs/audit/cross_engine_comparison_2026_09_23.md, evidence in docs/audit/cross_engine_2026_09_23/.
